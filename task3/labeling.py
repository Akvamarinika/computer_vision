...
def two_pass_labeling(B):
    ...
    return relabelling(labels)


def relabelling(labels):
    old_labels = list(set(labels.ravel()))[1:]
    new_labels = dict(zip(old_labels, [i for i in range(1, len(old_labels)+1)]))

    for row in range(labels.shape[0]):
        for col in range(labels.shape[1]):
            if labels[row, col] in new_labels:
                labels[row, col] = new_labels.get(labels[row, col])
    return labels
